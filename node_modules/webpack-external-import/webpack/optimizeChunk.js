...
function wrapChunks(compilation, chunks) {
  ...
  chunks.forEach(function (chunk) {
    console.log("chunk", chunk); // map weak maps and weak sets for better organization & perf
    // console.group(group)

    console.log("chunk", chunk.id, chunk.canBeInitial(), chunk.isOnlyInitial(), chunk.hasEntryModule(), chunk.hasRuntime()); // check if this chunk is an entrypoint or has the webpack runtime
      ...
      module.reasons.forEach(function (reason) {
        if (reason.module) {
          ...
          reason.module.chunksIterable.forEach(function (reasonChunk) {
            ...
            reasonChunk.files.forEach(function (file) {
              if (file.includes(".css")) {
                // convert these to sets
    ...
    Object.keys(orgs).forEach(function (key) {
      Array.from(orgs[key].js).forEach(function (subSet) {
        if (orgs[subSet]) {
          var _orgs$subSet$js;
          ...
          if (!map.ignoredChunk.has(subSet)) {
            ...
            (_orgs$key$js = orgs[key].js).add.apply(_orgs$key$js, _toConsumableArray(orgs[subSet].js));
  ...
  console.log("internal org", orgs); // to ensure the chunk maps are complete, i run another loop over the chunks - the previous loop creates a complete map
    ...
    try {
      for (var _iterator = chunk.files[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        if (ModuleFilenameHelpers.matchObject({}, fileName) && fileName.indexOf(".js") !== -1) {
          ...
          var AllChunksNeeded = Array.from(orgs[chunk.id].js); // create the final map which contains an array of chunkID as well as a object of chunk of what each chunk needs
